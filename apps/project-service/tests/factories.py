...
from packages.common.testing.base_factory import (BaseFactory,
                                                  SequentialNameMixin,
                                                  TimestampMixin)
...
class ProjectFactory(BaseFactory, SequentialNameMixin, TimestampMixin):
    ...
    class Params:
        ...
        active = factory.Trait(status="active", is_public=True, is_archived=False)

        # In progress project trait
        in_progress = factory.Trait(
            status="in_progress", is_public=False, is_archived=False
        )

        # Completed project trait
        completed = factory.Trait(status="completed", is_public=True, is_archived=False)

        # Archived project trait
        archived = factory.Trait(status="archived", is_public=False, is_archived=True)
...
class CommentFactory(BaseFactory, TimestampMixin):
        ...
        if extracted:
            if isinstance(extracted, int):
                ...
                CommentFactory.create_batch(
                    extracted, parent=self, project=self.project, **kwargs
                )
            elif isinstance(extracted, list):
    ...
    class Params:
        ...
        top_level = factory.Trait(parent_id=None, parent=None)

        # Reply comment (has parent)
        reply = factory.Trait(
            parent=factory.SubFactory(
                "tests.factories.CommentFactory",
...
def create_project_with_collaborators(
    session, num_collaborators: int = 3, **project_kwargs
):
    """
