...
class Review(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_reviewee_type_status_rating', 'reviewee_id', 'review_type', 'status', 'overall_rating'),
        ...
        Index('idx_review_status_rating_created', 'status', 'overall_rating', 'created_at'),  # For public reviews
        Index('idx_helpful_votes_status', 'helpful_votes', 'status'),  # For popular reviews
        Index('idx_moderation_queue', 'status', 'created_at',
              postgresql_where="status = 'PENDING_MODERATION'"),  # For moderation
        
        # Unique constraint to prevent duplicate reviews
        Index('idx_unique_booking_reviewer_type', 'booking_id', 'reviewer_id', 'review_type', unique=True),
    )
    ...
    reviewer_id = Column(Integer, nullable=False, index=True)  # Who is writing the review
    ...
    status = Column(SQLEnum(ReviewStatus), nullable=False, default=ReviewStatus.PUBLISHED)
    ...
    booking = relationship(
        "Booking",
        overlaps="reviews"  # Silence SQLAlchemy warning
    )
    
    @validates('overall_rating', 'quality_rating', 'timeliness_rating', 'communication_rating')
    ...
    def moderate_review(self, moderator_id: int, status: ReviewStatus):
        ...
        if status not in [ReviewStatus.PUBLISHED, ReviewStatus.REJECTED, ReviewStatus.HIDDEN]:
    ...
    @classmethod
    def get_provider_rating_summary(cls, provider_id: int, db_session) -> Dict[str, Any]:
        ...
        rating_breakdown_query = db_session.query(
            cls.overall_rating,
            ...
            cls.review_type == ReviewType.SEEKER_TO_PROVIDER,
            cls.status == ReviewStatus.PUBLISHED
        ).group_by(cls.overall_rating).all()
