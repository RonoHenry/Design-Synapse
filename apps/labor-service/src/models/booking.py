...
class Booking(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_provider_status_schedule', 'provider_id', 'status', 'scheduled_start_date'),
        Index('idx_client_status_schedule', 'client_id', 'status', 'scheduled_start_date'),
        Index('idx_schedule_range', 'scheduled_start_date', 'scheduled_completion_date', 'status'),
        
        # Performance optimization indexes
        Index('idx_booking_status_created', 'status', 'created_at'),  # For status-based queries
        Index('idx_quote_booking', 'quote_id', 'status'),  # For quote-to-booking tracking
        Index('idx_service_request_booking', 'service_request_id', 'status'),  # For request tracking
        
        # Calendar and scheduling optimization
        Index('idx_active_schedule', 'scheduled_start_date', 'scheduled_completion_date',
    ...
    cancellation_penalty = Column(Numeric(10, 2), nullable=True, default=Decimal("0.00"))
    ...
    def validate_schedule(self, key, value):
        ...
        if key == 'scheduled_completion_date' and hasattr(self, 'scheduled_start_date'):
            ...
            if self.scheduled_start_date and value and value <= self.scheduled_start_date:
...
class BookingMilestone(Base):
    ...
    __table_args__ = (
        ...
        # Performance optimization indexes
        Index('idx_due_date_pending', 'due_date', 'status',
              postgresql_where="status = 'PENDING'"),  # For pending milestones
        Index('idx_completed_date', 'completed_date', 'status'),  # For completion tracking
        
        # Unique constraint for sequence within booking
        Index('idx_unique_booking_sequence', 'booking_id', 'sequence_number', unique=True),
    )
    ...
    def is_overdue(self) -> bool:
        ...
        if not self.due_date or self.status in [MilestoneStatus.COMPLETED, MilestoneStatus.PAID]:
            return False
