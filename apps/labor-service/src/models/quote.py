...
class Quote(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_status_expiration", "status", "valid_until"
        ),  # For expiration cleanup
        Index(
            "idx_cost_range_status", "total_cost", "status"
        ),  # For cost-based filtering
        Index(
            "idx_submitted_date", "submitted_at", "status"
    ...
    service_request = relationship(
        "ServiceRequest", overlaps="quotes"  # Silence SQLAlchemy warning
    )
    ...
    def __init__(self, **kwargs):
        ...
        if (
            hasattr(self, "labor_cost")
            and hasattr(self, "material_cost")
            and hasattr(self, "travel_cost")
