...
class ServiceRequest(Base):
    ...
    __table_args__ = (
        ...
        Index('idx_status_created', 'status', 'created_at'),  # For active requests by date
        Index('idx_location_urgency', 'location_latitude', 'location_longitude', 'urgency_level'),
        Index('idx_budget_range_status', 'budget_min', 'budget_max', 'status'),  # For budget filtering
        
        # Performance optimization indexes
        Index('idx_project_status', 'project_id', 'status'),  # For project integration
        Index('idx_urgency_created', 'urgency_level', 'created_at'),  # For urgent requests
    ...
    requirements = Column(JSON, nullable=True)  # Detailed requirements as structured data
...
class SkillRequirement(Base):
    ...
    skill_id = Column(Integer, nullable=False, index=True)  # References Skills from ServiceProvider
    
    # Requirement details
    proficiency_level = Column(String(20), nullable=False, default="INTERMEDIATE")  # BEGINNER, INTERMEDIATE, ADVANCED, EXPERT
    is_required = Column(Boolean, nullable=False, default=True)  # Required vs. preferred
    importance_weight = Column(Integer, nullable=False, default=5)  # 1-10 scale for matching algorithms
    ...
    # Relationships
    service_request = relationship("ServiceRequest", back_populates="skill_requirements")
