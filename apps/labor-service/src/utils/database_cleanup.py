...
class DatabaseCleaner:
    ...
    def clean_test_data(self) -> Dict[str, Any]:
        ...
        test_providers = (
            ...
            .filter(ServiceProvider.user_id >= 1000)
            .all()
        )
        ...
        if test_providers:
            ...
            review_count = (
                ...
                .filter(Review.reviewee_id.in_(provider_ids))
                .count()
            )
            if review_count > 0:
            ...
            if booking_count > 0:
                ...
                milestone_count = (
                    self.session.query(BookingMilestone)
                    .join(Booking)
                    .filter(Booking.provider_id.in_(provider_ids))
                    .count()
                )
            ...
            quote_count = (
                ...
                .filter(Quote.provider_id.in_(provider_ids))
                .count()
            )
            if quote_count > 0:
            ...
            provider_skill_count = (
                self.session.query(ProviderSkill)
                .filter(ProviderSkill.provider_id.in_(provider_ids))
                .count()
            )
            ...
            # Clean service areas
            service_area_count = (
                self.session.query(ServiceArea)
                .filter(ServiceArea.provider_id.in_(provider_ids))
                .count()
            )
        ...
        test_requests = (
            ...
            .filter(ServiceRequest.seeker_id >= 1000)
            .all()
        )
        ...
        if test_requests:
            ...
            skill_req_count = (
                self.session.query(SkillRequirement)
                .filter(SkillRequirement.service_request_id.in_(request_ids))
                .count()
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "clean-all":
            ...
            print(
                "Usage: python database_cleanup.py [clean-all|clean-test|vacuum] [--confirm]"
            )
    else:
        print(
            "Usage: python database_cleanup.py [clean-all|clean-test|vacuum] [--confirm]"
