...
class ReviewRepository(BaseRepository[Review]):
    ...
    def find_by_booking_id(self, booking_id: int) -> List[Review]:
        ...
        return self.db_session.query(Review).filter(
    ...
    def find_by_rating_range(self, min_rating: int, max_rating: int = 5) -> List[Review]:
    ...
    def find_provider_reviews(self, provider_id: int, status: ReviewStatus = ReviewStatus.PUBLISHED) -> List[Review]:
    ...
    def find_seeker_reviews(self, seeker_id: int, status: ReviewStatus = ReviewStatus.PUBLISHED) -> List[Review]:
    ...
    def get_provider_rating_summary(self, provider_id: int) -> Dict[str, Any]:
        ...
        recent_reviews = [r for r in reviews if _make_timezone_aware(r.created_at) >= thirty_days_ago]
    ...
    def find_pending_reviews(self) -> List[Review]:
        ...
        ).order_by(asc(Review.created_at)).all()
    ...
    def find_high_rated_reviews(self, min_rating: int = 4, limit: int = 20) -> List[Review]:
    ...
    def find_low_rated_reviews(self, max_rating: int = 2, limit: int = 20) -> List[Review]:
        ...
        ).order_by(asc(Review.rating), desc(Review.created_at)).limit(limit).all()
    
    def search_reviews(self, search_term: str, review_type: ReviewType = None) -> List[Review]:
    ...
    def get_review_statistics(self, start_date: datetime = None, end_date: datetime = None) -> Dict[str, Any]:
        ...
        provider_reviews = [r for r in reviews if r.review_type == ReviewType.PROVIDER_REVIEW]
        seeker_reviews = [r for r in reviews if r.review_type == ReviewType.SEEKER_REVIEW]
    ...
    def find_mutual_reviews(self, user_id_1: int, user_id_2: int) -> List[Review]:
        ...
        return self.db_session.query(Review).filter(
            or_(
                and_(
                    Review.reviewer_id == user_id_1,
                    Review.reviewee_id == user_id_2
                ),
                and_(
                    Review.reviewer_id == user_id_2,
                    Review.reviewee_id == user_id_1
                )
            )
        ).order_by(desc(Review.created_at)).all()
    
    async def update_review_status(self, review_id: int, new_status: ReviewStatus, moderator_notes: str = None) -> Optional[Review]:
    ...
    def get_top_reviewers(self, limit: int = 10, min_reviews: int = 5) -> List[Dict[str, Any]]:
        ...
        reviewer_stats = self.db_session.query(
            Review.reviewer_id,
            func.count(Review.id).label('review_count'),
            func.avg(Review.overall_rating).label('avg_rating_given')
        ).filter(
            Review.status == ReviewStatus.PUBLISHED
        ).group_by(Review.reviewer_id).having(
        ...
        return [
            {
                ...
                "avg_rating_given": float(stat.avg_rating_given) if stat.avg_rating_given else 0.0
            }
            for stat in reviewer_stats
        ]
    
    async def get_by_booking_and_reviewer(self, booking_id: int, reviewer_id: int, review_type: ReviewType) -> Optional[Review]:
    ...
    async def get_provider_reviews(self, provider_id: int, limit: int = 50, offset: int = 0, min_rating: Optional[int] = None) -> List[Review]:
    ...
    async def get_seeker_reviews(self, seeker_id: int, limit: int = 50, offset: int = 0) -> List[Review]:
    ...
    async def get_review_analytics(self, provider_id: Optional[int] = None, start_date: Optional[datetime] = None, end_date: Optional[datetime] = None) -> Dict[str, Any]:
        """Get review analytics."""
        query = self.db_session.query(Review).filter(Review.status == ReviewStatus.PUBLISHED)
    ...
    async def search_by_rating(self, min_rating: int, max_rating: int, limit: int = 50, offset: int = 0) -> List[Review]:
    ...
    async def get_recent_reviews(self, since_date: datetime = None, limit: int = 50, days_back: int = None) -> List[Review]:
    ...
    def get_by_reviewee_id(self, reviewee_id: int, limit: int = 50, offset: int = 0) -> List[Review]:
    ...
    async def add_response(self, review_id: int, response_text: str, responder_id: int) -> None:
    ...
    def search_by_criteria(self, criteria: Dict[str, Any]) -> List[Review]:
