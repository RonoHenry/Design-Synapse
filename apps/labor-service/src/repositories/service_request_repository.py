...
class ServiceRequestRepository(BaseRepository[ServiceRequest]):
    ...
    def find_by_status(self, status: RequestStatus) -> List[ServiceRequest]:
        ...
        return self.db_session.query(ServiceRequest).filter(
    ...
    def find_by_urgency(self, urgency: UrgencyLevel) -> List[ServiceRequest]:
        ...
        return self.db_session.query(ServiceRequest).filter(
            ServiceRequest.urgency_level == urgency
        ).all()
    
    def find_by_budget_range(self, min_budget: float, max_budget: float) -> List[ServiceRequest]:
    ...
    def find_by_location(self, latitude: float, longitude: float, radius_km: float = 50) -> List[ServiceRequest]:
    ...
    def find_by_skills(self, skill_ids: List[int]) -> List[ServiceRequest]:
        ...
        return self.db_session.query(ServiceRequest).join(
            SkillRequirement
        ).filter(
            SkillRequirement.skill_id.in_(skill_ids)
        ).distinct().all()
    ...
    def find_matching_requests(self, 
                             ...
                             location: tuple = None,
                             radius_km: float = 50) -> List[ServiceRequest]:
    ...
    def get_with_skills(self, request_id: int) -> Optional[ServiceRequest]:
        ...
        return self.db_session.query(ServiceRequest).options(
            joinedload(ServiceRequest.skill_requirements).joinedload(SkillRequirement.skill)
    ...
    def get_by_seeker_id(self, seeker_id: int) -> List[ServiceRequest]:
        ...
        return self.db_session.query(ServiceRequest).filter(
            ServiceRequest.seeker_id == seeker_id
        ).order_by(ServiceRequest.created_at.desc()).all()
    
    def search_by_location(self, latitude: float, longitude: float, radius_km: float = 50) -> List[ServiceRequest]:
