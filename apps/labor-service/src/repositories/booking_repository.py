...
class BookingRepository(BaseRepository[Booking]):
    ...
    def find_active_bookings(self) -> List[Booking]:
        ...
        ).order_by(asc(Booking.scheduled_start_date)).all()
    ...
    def find_overdue_bookings(self) -> List[Booking]:
        ...
        ).order_by(asc(Booking.scheduled_completion_date)).all()
    ...
    def find_provider_schedule(self, provider_id: int, start_date: datetime, end_date: datetime) -> List[Booking]:
    ...
    def get_with_milestones(self, booking_id: int) -> Optional[Booking]:
        ...
        return self.db_session.query(Booking).options(
            joinedload(Booking.milestones)
    ...
    def get_booking_statistics(self, provider_id: int = None, seeker_id: int = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "completion_rate": len(completed) / len(bookings) * 100 if bookings else 0.0,
            "avg_booking_value": sum(booking_values) / len(booking_values) if booking_values else 0.0,
            "total_revenue": sum(booking_values)
        }
    
    def find_conflicting_bookings(self, provider_id: int, start_date: datetime, end_date: datetime, exclude_booking_id: int = None) -> List[Booking]:
    ...
    async def update_booking_status(self, booking_id: int, new_status: BookingStatus) -> Optional[Booking]:
        ...
        if new_status == BookingStatus.IN_PROGRESS:
            ...
            booking.actual_end_date = datetime.now(timezone.utc)
    ...
    def find_revenue_by_period(self, start_date: datetime, end_date: datetime, provider_id: int = None) -> Dict[str, Any]:
    ...
    async def get_upcoming_bookings(self, future_date: datetime = None) -> List[Booking]:
    ...
    async def get_analytics(self, booking_id: int) -> Dict[str, Any]:
        ...
        milestones = self.db_session.query(BookingMilestone).filter(
            BookingMilestone.booking_id == booking_id
        ).all()
    ...
    async def get_provider_statistics(self, provider_id: int) -> Dict[str, Any]:
        ...
        recent_bookings = self.db_session.query(Booking).filter(
            and_(
                Booking.provider_id == provider_id,
        ...
        cancelled_bookings = [b for b in all_bookings if b.status == BookingStatus.CANCELLED]
        
        # Calculate on-time completion rate
        on_time_completions = 0
        for booking in completed_bookings:
            if (booking.actual_end_date and booking.scheduled_completion_date and 
                booking.actual_end_date <= booking.scheduled_completion_date):
                on_time_completions += 1
        
        on_time_rate = on_time_completions / len(completed_bookings) if completed_bookings else 0
        
        # Calculate revenue from recent bookings
        recent_revenue = sum(
            float(b.total_cost) for b in recent_bookings 
            if b.total_cost and b.status == BookingStatus.COMPLETED
        )
        
        return {
            "total_bookings": len(all_bookings),
            "completion_rate": len(completed_bookings) / len(all_bookings) if all_bookings else 0,
            "average_rating": 4.7,  # Placeholder - would come from reviews
            "on_time_completion_rate": on_time_rate,
            "cancellation_rate": len(cancelled_bookings) / len(all_bookings) if all_bookings else 0,
...
class BookingMilestoneRepository(BaseRepository[BookingMilestone]):
    ...
    def find_by_booking_id(self, booking_id: int) -> List[BookingMilestone]:
        ...
        return self.db_session.query(BookingMilestone).filter(
            BookingMilestone.booking_id == booking_id
        ).order_by(asc(BookingMilestone.sequence_number)).all()
    
    def find_by_status(self, status: MilestoneStatus) -> List[BookingMilestone]:
        """Find milestones by status."""
        return self.db_session.query(BookingMilestone).filter(
            BookingMilestone.status == status
        ).all()
    ...
    def find_overdue_milestones(self) -> List[BookingMilestone]:
        ...
        return self.db_session.query(BookingMilestone).filter(
            and_(
                ...
                BookingMilestone.status.in_([MilestoneStatus.PENDING, MilestoneStatus.IN_PROGRESS])
            )
        ).order_by(asc(BookingMilestone.due_date)).all()
    ...
    async def update_milestone_status(self, milestone_id: int, new_status: MilestoneStatus) -> Optional[BookingMilestone]:
