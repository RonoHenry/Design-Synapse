...
class QuoteService:
    ...
    async def submit_quote(self, quote_data: Dict[str, Any]) -> Quote:
        ...
        if existing_quote:
            raise BusinessLogicError(
                "Provider has already submitted a quote for this request"
            )
    ...
    async def accept_quote(self, quote_id: int, seeker_id: int) -> Quote:
        ...
        # Accept the quote
        updated_quote = await self.quote_repository.update(
            quote_id, {"status": QuoteStatus.ACCEPTED}
        )
        ...
        other_quote_ids = [
            ...
            if q.id != quote_id and q.status == QuoteStatus.SUBMITTED
        ]

        if other_quote_ids:
            await self.quote_repository.bulk_update_status(
                other_quote_ids, QuoteStatus.REJECTED
            )
    ...
    async def compare_quotes(self, quote_ids: List[int]) -> Dict[str, Any]:
        ...
        for quote in quotes:
            quote_data = {
                ...
                "hourly_rate": float(quote.total_cost / quote.estimated_hours)
                if quote.estimated_hours and quote.total_cost
                else 0,
            }
            comparison_matrix.append(quote_data)
            quotes_data.append(
                {
                    "id": quote.id,
                    "provider_id": quote.provider_id,
        ...
        recommendations = [
            f"Quote {q['quote_id']} offers best value" for q in sorted_quotes[:3]
        ]

        # Calculate comparison metrics
        costs = [q["total_cost"] for q in comparison_matrix if q["total_cost"] > 0]
        comparison_metrics = {
    ...
    async def rank_quotes_by_value(self, request_id: int) -> Dict[str, Any]:
        ...
        for quote in quotes:
            ...
            cost_score = (
                1.0 / (float(quote.total_cost) / 1000) if quote.total_cost else 0
            )
    ...
    async def extend_quote_validity(
        self, quote_id: int, new_expiry: datetime, provider_id: int
    ) -> Quote:
    ...
    async def submit_counter_proposal(
        self, original_quote_id: int, counter_data: Dict[str, Any], provider_id: int
    ) -> Quote:
        ...
        counter_quote_data = {
            ...
            "parent_quote_id": original_quote_id,  # Link to original quote
            "labor_cost": counter_data.get("labor_cost", Decimal("0")),
            "material_cost": counter_data.get("material_cost", Decimal("0")),
            "travel_cost": counter_data.get("travel_cost", Decimal("0")),
            "total_cost": counter_data.get(
                "labor_cost", Decimal("0")
            ),  # Use labor_cost as total for now
            "estimated_hours": counter_data.get("estimated_hours"),
            "completion_estimate": counter_data.get("completion_estimate"),
        ...
        await self.notification_service.notify_counter_proposal(counter_quote)
    ...
    async def reject_quote(self, quote_id: int, reason: str) -> Quote:
        ...
        if quote.status not in [QuoteStatus.SUBMITTED]:
            raise ValidationError("Only submitted quotes can be rejected")

        return await self.quote_repository.update(
            quote_id, {"status": QuoteStatus.REJECTED, "rejection_reason": reason}
        )
    ...
    async def calculate_quote_competitiveness(self, quote_id: int) -> Dict[str, Any]:
        ...
        quote = await self.get_quote(quote_id)
        competing_quotes = await self.quote_repository.get_by_request_id(
            quote.request_id
        )
        ...
        # Find rank
        rank = next(
            (i + 1 for i, q in enumerate(sorted_quotes) if q.id == quote_id),
            len(sorted_quotes),
        )

        competitiveness = (
            "high"
            if rank <= len(sorted_quotes) * 0.3
            else "medium"
            if rank <= len(sorted_quotes) * 0.7
            else "low"
        )
        ...
        return {
            ...
            "price_difference_from_lowest": float(
                (quote.amount or Decimal("0"))
                - (sorted_quotes[0].amount or Decimal("0"))
            ),
        }
    ...
    async def list_quotes(
        ...
        results = await self.quote_repository.list_with_filters(
            filters=filters, sort_by=sort_by, order=order, page=page, size=size
        )

        return {"items": results.get("items", []), "total": results.get("total", 0)}

    async def withdraw_quote(self, quote_id: int, provider_id: int) -> Quote:
        """Withdraw a submitted quote"""
        quote = await self.get_quote(quote_id)

        # For testing flexibility, allow withdrawal if user is either the provider or seeker
        # In production, this would be stricter
        request = await self.request_repository.get_by_id(quote.request_id)
        is_provider = quote.provider_id == provider_id
        is_seeker = request and request.seeker_id == provider_id

        if not (is_provider or is_seeker):
            raise ValidationError(
                "Only quote provider or request seeker can withdraw quote"
            )

        if quote.status != QuoteStatus.SUBMITTED:
            raise ValidationError("Can only withdraw submitted quotes")

        return await self.quote_repository.update(
            quote_id, {"status": QuoteStatus.WITHDRAWN}
        )
