...
from src.models import (
    ServiceRequest, SkillRequirement, Quote, Booking, 
    BookingMilestone, Review, ServiceProvider,
    SkillCategory, Skill, ProviderSkill, ServiceArea,
    UrgencyLevel, RequestStatus, QuoteStatus, 
    BookingStatus, MilestoneStatus, ReviewType, ReviewStatus,
...
class ProviderSkillFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    hourly_rate = factory.Faker('pydecimal', left_digits=3, right_digits=2, min_value=25, max_value=150)
...
class ServiceAreaFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    travel_rate = factory.Faker('pydecimal', left_digits=2, right_digits=2, min_value=0.50, max_value=2.00)
...
class ServiceRequestFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    # Timing and urgency
    urgency_level = factory.Faker('random_element', elements=[level for level in UrgencyLevel])
    preferred_start_date = factory.LazyFunction(lambda: datetime.now() + timedelta(days=7))
...
class SkillRequirementFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    proficiency_level = factory.Faker('random_element', elements=["BEGINNER", "INTERMEDIATE", "ADVANCED", "EXPERT"])
...
class QuoteFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    # Cost breakdown
    labor_cost = factory.LazyFunction(lambda: Decimal(str(fake.random_int(min=200, max=2000))))
    material_cost = factory.LazyFunction(lambda: Decimal(str(fake.random_int(min=0, max=1000))))
    travel_cost = factory.LazyFunction(lambda: Decimal(str(fake.random_int(min=0, max=200))))
    
    # Timeline
    start_availability = factory.LazyFunction(lambda: datetime.now() + timedelta(days=fake.random_int(min=1, max=14)))
    completion_estimate = factory.LazyAttribute(lambda obj: obj.start_availability + timedelta(days=fake.random_int(min=1, max=21)))
    ...
    def set_completion_estimate(self, create, extracted, **kwargs):
        ...
        if self.start_availability and self.estimated_hours:
            ...
            self.completion_estimate = self.start_availability + timedelta(days=days_needed)
...
class BookingFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    scheduled_start_date = factory.LazyFunction(lambda: datetime.now() + timedelta(days=fake.random_int(min=1, max=30)))
    scheduled_completion_date = factory.LazyAttribute(lambda obj: obj.scheduled_start_date + timedelta(days=fake.random_int(min=1, max=7)))
    
    # Cost information
    total_cost = factory.LazyFunction(lambda: Decimal(str(fake.random_int(min=500, max=5000))))
    currency = "USD"
    payment_terms = factory.LazyFunction(lambda: {
...
class BookingMilestoneFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    booking_id = factory.Sequence(lambda n: n + 1000)  # Default booking_id, can be overridden
    title = factory.Faker('sentence', nb_words=3)
    description = factory.Faker('text', max_nb_chars=300)
    sequence_number = factory.Sequence(lambda n: n + 1)
    
    # Payment information
    amount = factory.LazyFunction(lambda: Decimal(str(fake.random_int(min=100, max=1000))))
    currency = "USD"
    ...
    # Dates
    due_date = factory.LazyFunction(lambda: datetime.now() + timedelta(days=fake.random_int(min=7, max=30)))
...
class ReviewFactory(factory.alchemy.SQLAlchemyModelFactory):
    ...
    # Review type
    review_type = factory.Faker('random_element', elements=[ReviewType.SEEKER_TO_PROVIDER, ReviewType.PROVIDER_TO_SEEKER])
...
class HighUrgencyServiceRequestFactory(ServiceRequestFactory):
    ...
    preferred_start_date = factory.LazyFunction(lambda: datetime.now() + timedelta(hours=24))
...
class CompletedBookingFactory(BookingFactory):
    ...
    actual_completion_date = factory.LazyFunction(lambda: datetime.now() - timedelta(days=1))
...
def create_provider_with_multiple_quotes(db_session, num_quotes: int = 3) -> Dict[str, Any]:
    """
