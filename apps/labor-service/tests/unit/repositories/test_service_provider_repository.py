...
from tests.factories import (ProviderSkillFactory, ServiceAreaFactory,
                             ServiceProviderFactory, SkillCategoryFactory,
                             SkillFactory)
...
class TestServiceProviderRepository:
    ...
    def test_find_by_verification_status(self, db_session):
        ...
        verified_providers = repo.find_by_verification_status(
    ...
    def test_find_by_provider_type(self, db_session):
        ...
        individual_provider = ServiceProviderFactory.create(
            provider_type=ProviderType.INDIVIDUAL, individual_name="John Doe"
        )
        business_provider = ServiceProviderFactory.create(
            provider_type=ProviderType.BUSINESS, business_name="ABC Construction"
        )
        db_session.commit()
    ...
    def test_find_by_skills(self, db_session):
        ...
        advanced_providers = repo.find_by_skills(
            [plumbing_skill.id], ProficiencyLevel.ADVANCED
        )
        ...
        advanced_electrical = repo.find_by_skills(
            [electrical_skill.id], ProficiencyLevel.ADVANCED
        )
    ...
    def test_find_by_rating_range(self, db_session):
        ...
        high_rated_provider = ServiceProviderFactory.create(
            rating=Decimal("4.8"), business_name="High Rated"
        )
        low_rated_provider = ServiceProviderFactory.create(
            rating=Decimal("3.2"), business_name="Low Rated"
        )
        db_session.commit()
    ...
    def test_find_top_rated_providers(self, db_session):
        ...
        top_provider = ServiceProviderFactory.create(
            rating=Decimal("4.9"), total_reviews=50, business_name="Top Provider"
        )
        good_provider = ServiceProviderFactory.create(
            rating=Decimal("4.2"), total_reviews=25, business_name="Good Provider"
        )
        no_reviews_provider = ServiceProviderFactory.create(
            rating=Decimal("0.0"), total_reviews=0, business_name="No Reviews"
        )
        db_session.commit()
    ...
    def test_search_providers(self, db_session):
        ...
        assert (
            len(plumbing_providers) == 2
        )  # Should not include inactive provider or non-matching terms
