...
class TestReviewRepository:
    ...
    def test_find_by_booking_id(self, db_session):
        ...
        review_2 = ReviewFactory.create(
            booking_id=1001, title="Review 2", created_at=datetime.now(timezone.utc)
        )
        other_booking_review = ReviewFactory.create(
            booking_id=1002, title="Other Booking"
        )
        db_session.commit()
    ...
    def test_find_by_reviewer_id(self, db_session):
        ...
        reviewer_review_1 = ReviewFactory.create(
            reviewer_id=3001, title="Reviewer Review 1"
        )
        reviewer_review_2 = ReviewFactory.create(
            reviewer_id=3001, title="Reviewer Review 2"
        )
        other_reviewer_review = ReviewFactory.create(
            reviewer_id=3002, title="Other Reviewer"
        )
        db_session.commit()
    ...
    def test_find_by_reviewee_id(self, db_session):
        ...
        # Create test data
        reviewee_review_1 = ReviewFactory.create(reviewee_id=4001, title="About User 1")
        reviewee_review_2 = ReviewFactory.create(reviewee_id=4001, title="About User 2")
        other_reviewee_review = ReviewFactory.create(
            reviewee_id=4002, title="About Other User"
        )
        db_session.commit()
    ...
    def test_find_by_review_type(self, db_session):
        ...
        provider_review = ReviewFactory.create(
            review_type=ReviewType.SEEKER_TO_PROVIDER, title="Provider Review"
        )
        seeker_review = ReviewFactory.create(
            review_type=ReviewType.SEEKER_REVIEW, title="Seeker Review"
        )
        db_session.commit()
    ...
    def test_find_by_status(self, db_session):
        ...
        published_review = ReviewFactory.create(
            status=ReviewStatus.PUBLISHED, title="Published Review"
        )
        pending_review = ReviewFactory.create(
            status=ReviewStatus.PENDING, title="Pending Review"
        )
        db_session.commit()
    ...
    def test_find_by_rating_range(self, db_session):
        ...
        # Create test data
        high_rating_review = ReviewFactory.create(rating=5, title="Excellent")
        mid_rating_review = ReviewFactory.create(rating=3, title="Average")
        low_rating_review = ReviewFactory.create(rating=1, title="Poor")
        db_session.commit()
    ...
    def test_find_pending_reviews(self, db_session):
        ...
        published_review = ReviewFactory.create(
            status=ReviewStatus.PUBLISHED, title="Published"
        )
        db_session.commit()
    ...
    def test_search_reviews(self, db_session):
        ...
        pending_match = ReviewFactory.create(
            status=ReviewStatus.PENDING, title="Plumbing review", comment="Good work"
        )
        db_session.commit()
    ...
    def test_find_mutual_reviews(self, db_session):
        ...
        user_1_about_2 = ReviewFactory.create(
            reviewer_id=1001, reviewee_id=1002, title="User 1 about User 2"
        )
        user_2_about_1 = ReviewFactory.create(
            reviewer_id=1002, reviewee_id=1001, title="User 2 about User 1"
        )
        unrelated_review = ReviewFactory.create(
            reviewer_id=1003, reviewee_id=1004, title="Unrelated"
        )
        db_session.commit()
    ...
    async def test_update_review_status(self, db_session):
        ...
        # Create test data
        review = ReviewFactory.create(status=ReviewStatus.PENDING, title="Test Review")
        db_session.commit()

        # Test updating to PUBLISHED
        updated_review = await repo.update_review_status(
            review.id, ReviewStatus.PUBLISHED, "Approved by moderator"
        )
    ...
    def test_get_top_reviewers(self, db_session):
        ...
        for i in range(6):
            ReviewFactory.create(
                reviewer_id=1001, status=ReviewStatus.PUBLISHED, rating=4
            )

        # Reviewer 2: 5 reviews, avg rating 5.0
        for i in range(5):
            ReviewFactory.create(
                reviewer_id=1002, status=ReviewStatus.PUBLISHED, rating=5
            )

        # Reviewer 3: 3 reviews (below minimum)
        for i in range(3):
            ReviewFactory.create(
                reviewer_id=1003, status=ReviewStatus.PUBLISHED, rating=3
            )
