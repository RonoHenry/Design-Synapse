...
from src.repositories.booking_repository import BookingRepository, BookingMilestoneRepository
...
class TestBookingRepository:
    ...
    def test_find_by_client_id(self, db_session):
        ...
        seeker_booking_1 = BookingFactory.create(
            client_id=3001,
            notes="Seeker Booking 1"
        )
        seeker_booking_2 = BookingFactory.create(
            client_id=3001,
            notes="Seeker Booking 2"
        )
    ...
    def test_find_by_date_range(self, db_session):
        ...
        in_range_booking = BookingFactory.create(
            scheduled_start_date=start_date + timedelta(days=3),
            notes="In Range"
        )
        before_range_booking = BookingFactory.create(
            scheduled_start_date=start_date - timedelta(days=1),
            notes="Before Range"
        )
        after_range_booking = BookingFactory.create(
            scheduled_start_date=end_date + timedelta(days=1),
            notes="After Range"
        )
        db_session.commit()
    ...
    async def test_update_booking_status(self, db_session):
        ...
        updated_booking = await repo.update_booking_status(booking.id, BookingStatus.IN_PROGRESS)
        assert updated_booking.status == BookingStatus.IN_PROGRESS
        assert updated_booking.actual_start_date is not None
        
        # Test updating to COMPLETED
        updated_booking = await repo.update_booking_status(booking.id, BookingStatus.COMPLETED)
    ...
    def test_get_booking_statistics(self, db_session):
        ...
        assert abs(stats["completion_rate"] - 33.333333333333336) < 0.0001  # 1/3 * 100 (floating point precision)
...
class TestBookingMilestoneRepository:
    ...
    def test_get_milestone_progress(self, db_session):
        ...
        assert abs(progress["progress_percentage"] - 33.333333333333336) < 0.0001  # 1/3 * 100 (floating point precision)
    ...
    async def test_update_milestone_status(self, db_session):
        ...
        # Test updating to COMPLETED
        updated_milestone = await repo.update_milestone_status(milestone.id, MilestoneStatus.COMPLETED)
