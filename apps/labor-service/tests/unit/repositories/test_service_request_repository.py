...
from tests.factories import (ServiceRequestFactory, SkillCategoryFactory,
                             SkillFactory)
...
class TestServiceRequestRepository:
    ...
    def test_find_by_status(self, db_session):
        ...
        open_request = ServiceRequestFactory.create(
            status=RequestStatus.ACTIVE, title="Open Request"
        )
        closed_request = ServiceRequestFactory.create(
            status=RequestStatus.COMPLETED, title="Closed Request"
        )
        db_session.commit()
    ...
    def test_find_by_urgency(self, db_session):
        ...
        urgent_request = ServiceRequestFactory.create(
            urgency_level=UrgencyLevel.EMERGENCY, title="Urgent Request"
        )
        normal_request = ServiceRequestFactory.create(
            urgency_level=UrgencyLevel.MEDIUM, title="Normal Request"
        )
        db_session.commit()
    ...
    def test_find_by_location(self, db_session):
        ...
        nyc_request = ServiceRequestFactory.create(
            location_latitude=40.7128, location_longitude=-74.0060, title="NYC Request"
        )
        # LA area (far from NYC)
        la_request = ServiceRequestFactory.create(
            location_latitude=34.0522, location_longitude=-118.2437, title="LA Request"
        )
        db_session.commit()
