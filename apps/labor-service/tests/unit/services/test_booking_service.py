...
import pytest
from src.core.exceptions import (BookingNotFoundError, BusinessLogicError,
                                 ValidationError)
...
class TestBookingService:
    ...
    def mock_notification_service(self):
        ...
        mock = Mock()
        # These methods are synchronous in the actual service
        mock.notify_booking_started = Mock()
        mock.notify_booking_completed = Mock()
        mock.notify_booking_cancelled = Mock()
        mock.notify_booking_rescheduled = Mock()
        mock.notify_progress_update = Mock()
        return mock
    ...
    def mock_payment_service(self):
        ...
        mock = Mock()
        # These methods are synchronous in the actual service
        mock.process_milestone_payment = Mock()
        mock.process_cancellation_refund = Mock()
        return mock
    ...
    def sample_request(self):
        ...
        return ServiceRequest(id=1, seeker_id=2, title="Kitchen Electrical Work")

    # Booking Creation Tests
    @pytest.mark.asyncio
    async def test_create_booking_from_accepted_quote(
        self,
        booking_service,
        mock_booking_repository,
        mock_quote_repository,
        mock_request_repository,
        sample_accepted_quote,
        sample_request,
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_booking_from_unaccepted_quote(
        self, booking_service, mock_quote_repository
    ):
        ...
        with pytest.raises(
            BusinessLogicError, match="Can only create booking from accepted quote"
        ):
            await booking_service.create_booking(1)

    @pytest.mark.asyncio
    async def test_create_booking_with_milestones(
        self,
        booking_service,
        mock_booking_repository,
        mock_quote_repository,
        ...
        booking_with_milestones = Booking(
            ...
            milestones=[
                BookingMilestone(
                    ...
                    sequence_number=i + 1,
                    amount=sample_accepted_quote.total_cost
        ...
        mock_booking_repository.create_with_milestones.return_value = (
            booking_with_milestones
        )
        ...
        assert (
            sum(m.amount for m in result.milestones) == sample_accepted_quote.total_cost
        )
    ...
    async def test_update_booking_status(
        self, booking_service, mock_booking_repository, mock_notification_service
    ):
        ...
        mock_booking_repository.update_booking_status = AsyncMock(
            return_value=updated_booking
        )

        result = await booking_service.update_booking_status(
            1, BookingStatus.IN_PROGRESS, user_id=2
        )

        assert result.status == BookingStatus.IN_PROGRESS
        mock_booking_repository.update_booking_status.assert_called_once()
    ...
    async def test_start_booking(
        ...
        # Notification services are synchronous, not called with await
    ...
    async def test_complete_booking(
        ...
        mock_booking_repository.update_booking_status = AsyncMock(
            return_value=completed_booking
        )
    ...
    async def test_complete_milestone(
        self, booking_service, mock_booking_repository, mock_payment_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_cannot_complete_future_milestone(
        self, booking_service, mock_booking_repository
    ):
        ...
        booking = Booking(
            ...
            milestones=[
                BookingMilestone(
                    id=1, sequence_order=1, status=MilestoneStatus.PENDING
                ),
                BookingMilestone(
                    id=2, sequence_order=2, status=MilestoneStatus.PENDING
                ),
            ],
        )
        mock_booking_repository.get_by_id.return_value = booking
        ...
        with pytest.raises(
            BusinessLogicError, match="Must complete milestones in order"
        ):
    ...
    @pytest.mark.asyncio
    async def test_cannot_reschedule_completed_booking(
        self, booking_service, mock_booking_repository
    ):
        """Test that completed bookings cannot be rescheduled"""
        completed_booking = Booking(id=1, status=BookingStatus.COMPLETED)
        ...
        with pytest.raises(
            BusinessLogicError, match="Cannot reschedule completed booking"
        ):
    ...
    async def test_cancel_booking(
        ...
        active_booking = Booking(
            id=1, status=BookingStatus.CONFIRMED, total_amount=Decimal("2500.00")
        )
        ...
        result = await booking_service.cancel_booking(
            1, "Client changed requirements", requester_id=2
        )
    ...
    @pytest.mark.asyncio
    async def test_cancel_booking_with_penalty(
        self, booking_service, mock_booking_repository, mock_payment_service
    ):
        ...
        # Mock the update_booking_status call
        cancelled_booking = Booking(
            id=1,
            status=BookingStatus.CANCELLED,
        ...
        result = await booking_service.cancel_booking(
            1, "Emergency cancellation", requester_id=2
        )
    ...
    @pytest.mark.asyncio
    async def test_get_bookings_by_provider(
        self, booking_service, mock_booking_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_bookings_by_seeker(
        self, booking_service, mock_booking_repository
    ):
    ...
    async def test_get_upcoming_bookings(
        ...
        expected_bookings = [
            Booking(
                id=1, scheduled_start=datetime.now(timezone.utc) + timedelta(days=2)
            ),
            Booking(
                id=2, scheduled_start=datetime.now(timezone.utc) + timedelta(days=5)
            ),
        ]
        ...
        assert len(result) == 2
        mock_booking_repository.get_upcoming_bookings.assert_called_once_with(
            upcoming_date
        )

    # Booking Analytics Tests
    @pytest.mark.asyncio
    async def test_get_booking_analytics(
        self, booking_service, mock_booking_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_get_provider_booking_statistics(
        self, booking_service, mock_booking_repository
    ):
        ...
        mock_booking_repository.get_provider_statistics.assert_called_once_with(
            provider_id
        )

    # Communication and Updates Tests
    @pytest.mark.asyncio
    async def test_add_booking_update(
        self, booking_service, mock_booking_repository, mock_notification_service
    ):
        ...
        result = await booking_service.add_progress_update(
            1, update_data, provider_id=1
        )
    ...
    @pytest.mark.asyncio
    async def test_booking_service_handles_repository_errors(
        self, booking_service, mock_booking_repository
    ):
        """Test that service properly handles repository errors"""
        mock_booking_repository.get_by_id.side_effect = Exception(
    ...
    @pytest.mark.asyncio
    async def test_validation_error_on_invalid_booking_data(
        self, booking_service, mock_quote_repository, mock_request_repository
    ):
        ...
        service_request = ServiceRequest(id=1, seeker_id=1, title="Test Request")
        ...
        with pytest.raises(ValidationError):
            await booking_service.create_booking_with_milestones(
                1, invalid_milestone_data
            )
