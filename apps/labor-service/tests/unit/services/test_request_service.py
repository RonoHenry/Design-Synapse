...
class TestRequestService:
    ...
    @pytest.fixture
    def request_service(
        self, mock_request_repository, mock_skill_repository, mock_notification_service
    ):
    ...
    async def test_create_request_success(
        self, request_service, mock_request_repository, sample_request_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_request_with_invalid_location(
        self, request_service, sample_request_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_create_request_with_invalid_budget(
        self, request_service, sample_request_data
    ):
        ...
        with pytest.raises(
            ValidationError, match="Budget maximum must be greater than minimum"
        ):
            await request_service.create_request(sample_request_data)
    ...
    async def test_create_emergency_request(
        ...
        sample_request_data["preferred_start_date"] = datetime.now(
            timezone.utc
        ) + timedelta(hours=2)
    ...
    async def test_get_request_success(self, request_service, mock_request_repository):
        ...
        expected_request = ServiceRequest(
            id=1, seeker_id=1, title="Test Request", status=RequestStatus.ACTIVE
        )
    ...
    async def test_get_request_not_found(
        self, request_service, mock_request_repository
    ):
    ...
    async def test_update_request_status(
        ...
        existing_request = ServiceRequest(
            id=1, seeker_id=1, title="Test Request", status=RequestStatus.DRAFT
        )
    ...
    async def test_publish_request(
        self, request_service, mock_request_repository, mock_notification_service
    ):
        ...
        assert result.status == RequestStatus.ACTIVE
        mock_request_repository.update.assert_called_once()
        mock_notification_service.notify_providers_of_new_request.assert_called_once_with(
            1
        )

    # Request Search and Filtering Tests
    @pytest.mark.asyncio
    async def test_search_requests_by_seeker(
        self, request_service, mock_request_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_search_requests_by_location(
        self, request_service, mock_request_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_search_requests_by_skills(
        self, request_service, mock_request_repository
    ):
        """Test searching requests by required skills"""
        skill_ids = [1, 2, 3]  # Electrical, Plumbing, Carpentry

        expected_requests = [ServiceRequest(id=1, title="Multi-skill Request")]
    ...
    @pytest.mark.asyncio
    async def test_validate_request_completeness(
        self, request_service, mock_request_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_validate_complete_request(
        self, request_service, mock_request_repository
    ):
    ...
    async def test_cancel_request(self, request_service, mock_request_repository):
        ...
        active_request = ServiceRequest(
            id=1, seeker_id=1, title="Active Request", status=RequestStatus.ACTIVE
        )
        mock_request_repository.get_by_id.return_value = active_request

        cancelled_request = ServiceRequest(
            id=1, seeker_id=1, title="Active Request", status=RequestStatus.CANCELLED
        )
    ...
    async def test_cannot_cancel_completed_request(
        ...
        completed_request = ServiceRequest(
            id=1, seeker_id=1, title="Completed Request", status=RequestStatus.COMPLETED
        )
    ...
    @pytest.mark.asyncio
    async def test_get_request_analytics(
        self, request_service, mock_request_repository
    ):
        ...
        mock_request_repository.get_analytics = AsyncMock(
