...
from src.models.service_provider import (ProficiencyLevel, ServiceProvider,
                                         VerificationStatus)
...
class TestMatchingService:
    ...
    @pytest.mark.asyncio
    async def test_find_providers_for_request(
        self,
        matching_service,
        mock_provider_repository,
        mock_request_repository,
    ...
    @pytest.mark.asyncio
    async def test_find_opportunities_for_provider(
        self, matching_service, mock_request_repository, sample_provider
    ):
        ...
        mock_request_repository.find_matching_requests.return_value = (
            expected_opportunities
        )
    ...
    async def test_calculate_match_score(
        self, matching_service, sample_provider, sample_request
    ):
        ...
        result = await matching_service.calculate_match_score(
            sample_provider.id, sample_request.id
        )
    ...
    async def test_skill_match_calculation(self, matching_service):
        ...
        result = matching_service._calculate_skill_match(
    ...
    async def test_skill_mismatch_calculation(self, matching_service):
        ...
        request_skills = [
            {
                "skill_id": 1,
                "required_level": ProficiencyLevel.EXPERT,
                "is_primary": True,
            },
            {
                "skill_id": 2,
    ...
    async def test_distance_score_calculation(self, matching_service):
        ...
        result = matching_service._calculate_distance_score(
    ...
    async def test_availability_match_calculation(self, matching_service):
        ...
        result = matching_service._calculate_availability_score(
            provider_availability, request_timing
        )
    ...
    async def test_send_job_alerts(self, matching_service, mock_notification_service):
        ...
        mock_notification_service.send_job_alerts.assert_called_once_with(
            provider_id, matching_requests
        )

    @pytest.mark.asyncio
    async def test_notify_providers_of_new_request(
        self, matching_service, mock_provider_repository, mock_notification_service
    ):
        ...
        mock_provider_repository.find_matching_providers.return_value = (
            matching_providers
        )
        ...
        expected_notifications = [
            p for p in matching_providers if p["match_score"] >= 0.85
        ]
        mock_notification_service.notify_providers.assert_called_once()

    # Matching Algorithm Configuration Tests
    @pytest.mark.asyncio
    async def test_emergency_request_matching(
        self, matching_service, mock_provider_repository
    ):
        ...
        mock_provider_repository.find_emergency_providers.return_value = (
            expected_matches
        )
    ...
    @pytest.mark.asyncio
    async def test_bulk_matching_for_project(
        self, matching_service, mock_provider_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_matching_performance_with_large_dataset(
        self, matching_service, mock_provider_repository
    ):
        """Test matching performance with large number of providers"""
        # Simulate large dataset
        large_provider_list = [
            {"provider_id": i, "match_score": 0.5 + (i % 50) / 100} for i in range(1000)
        ]
        mock_provider_repository.find_matching_providers.return_value = (
            large_provider_list
        )
    ...
    @pytest.mark.asyncio
    async def test_matching_with_invalid_request(
        self, matching_service, mock_request_repository
    ):
    ...
    @pytest.mark.asyncio
    async def test_matching_with_inactive_providers(
        self, matching_service, mock_provider_repository
    ):
