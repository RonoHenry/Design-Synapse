...
from sqlalchemy import (JSON, CheckConstraint, Column, DateTime, ForeignKey,
                        Integer, String, Table, Text, UniqueConstraint, event)
...
class Resource(Base):
    ...
    content_classification: Mapped[Optional[str]] = mapped_column(
    ...
    estimated_reading_time: Mapped[Optional[int]] = mapped_column(
    ...
    analysis_timestamp: Mapped[Optional[datetime]] = mapped_column(
        DateTime, nullable=True
    )
    ...
    topics: Mapped[List["Topic"]] = relationship(
        "Topic", secondary=resource_topics, back_populates="resources"
    )
    bookmarks: Mapped[List["Bookmark"]] = relationship(
        "Bookmark", back_populates="resource", cascade="all, delete-orphan"
    )
    citations: Mapped[List["Citation"]] = relationship(
        "Citation", back_populates="resource", cascade="all, delete-orphan"
    )
...
class Topic(Base):
    ...
    resources: Mapped[List["Resource"]] = relationship(
        "Resource", secondary=resource_topics, back_populates="topics"
    )
...
class Citation(Base):
    ...
    )  # Foreign key to project service
