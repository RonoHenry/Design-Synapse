...
from ..exceptions import (FileFormatError, FileSizeError, FileValidationError,
                          PDFProcessingError, TextExtractionError)
...
class PDFProcessingService(IPDFProcessingService):
    ...
    def __init__(self, storage_path: Optional[str] = None):
        ...
        logger.info(
            f"PDF processing service initialized with storage path: {self.storage_path}"
        )
    ...
    async def process_pdf(
        ...
        """
        with error_context(
            "pdf_validation", resource_id=resource.id, filename=file.filename
        ):
        ...
        try:
            with error_context(
                "pdf_storage", resource_id=resource.id, file_size=len(content)
            ):
            ...
            with error_context(
                "text_extraction", resource_id=resource.id, filepath=str(filepath)
            ):
                ...
                with error_context(
                    ...
                    if use_chunking:
                        ...
                        logger.info(
                            f"Indexed resource {resource.id} as single document"
                        )
    ...
    def _validate_file_extension(self, filename: str) -> None:
        ...
        if not any(
            filename.lower().endswith(ext) for ext in self.config.allowed_file_types
        ):
            raise FileFormatError(
                f"File must be one of: {', '.join(self.config.allowed_file_types)}"
            )
                        ...
                        page_content["blocks"].append(
                            {
                                "type": "text",
                                "content": "".join(
                                    [line["text"] for line in block.get("lines", [])]
                                ),
                                "bbox": block.get("bbox", []),
    ...
    async def generate_thumbnails(
        self, file_path: str, size: tuple = (200, 200)
    ) -> List[bytes]:
    ...
    def _process_content_for_indexing(self, text: str) -> str:
        ...
        processed_text = re.sub(
            r"^\s*\d+\s*$", "", processed_text, flags=re.MULTILINE
        ...
        max_length = (
            ...
            if hasattr(self.config, "max_content_length")
            else 50000
        )
