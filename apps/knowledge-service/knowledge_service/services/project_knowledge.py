...
class ProjectKnowledgeService:
    ...
    def _apply_database_filters(
        ...
        if source_platform:
            query_builder = query_builder.filter(
                func.lower(Resource.source_platform).contains(func.lower(source_platform))
            )
    ...
    def _sort_results(self, results: List[Dict], sort_by: str, sort_order: str) -> List[Dict]:
        ...
        if sort_by == "relevance":
            ...
            results.sort(
                key=lambda x: x.get("publication_date") or ("" if reverse else "9999-12-31"),
                reverse=reverse
            )
        elif sort_by == "title":
            ...
            results.sort(
                key=lambda x: x.get("author", "").lower() if x.get("author") else ("" if reverse else "zzz"),
                reverse=reverse
            )
        elif sort_by == "file_size":
            results.sort(
                key=lambda x: x.get("file_size", 0) if x.get("file_size") is not None else (0 if reverse else float('inf')),
                reverse=reverse
            )
        ...
        if include_global:
            ...
            try:
                ...
                for result in vector_results:
                    ...
                    if (resource_id in cited_ids or 
                        score < min_score or 
                        resource_id not in filtered_resource_ids):
                        continue
    ...
    async def search_global(
        ...
        try:
            ...
            vector_results = await self.vector_service.search_resources(
                query=query,
                filter_metadata=filter_metadata if filter_metadata else None,
                top_k=min(page_size * 10, 300)  # Get more results for sorting and filtering
            )
