...
from ..exceptions import (IndexingError, MetadataValidationError,
                          SearchQueryError, VectorSearchError)
    ...
    def set(
        ...
        try:
            if self.redis_client:
                ...
                logger.debug(
                    f"Cached search results in Redis for query: {query[:50]}..."
                )
            else:
                # In-memory cache
                ...
                self.access_times[key] = time.time()
                logger.debug(
                    f"Cached search results in memory for query: {query[:50]}..."
                )
            ...
            if self.redis_client:
                ...
                for key in keys_to_remove:
                    ...
                    if key in self.access_times:
                        del self.access_times[key]
                logger.info(
                    f"Invalidated {len(keys_to_remove)} in-memory cache entries matching pattern: {pattern}"
                )
        except Exception as e:
    ...
    def __init__(self, max_size: int = 500, redis_url: Optional[str] = None):
        ...
        if REDIS_AVAILABLE and redis_url:
            try:
                ...
                logger.warning(
                    f"Failed to initialize Redis embedding cache: {e}, falling back to in-memory"
                )
                self.redis_client = None
...
class VectorSearchService(IVectorSearchService):
    ...
    def __init__(self, config=None):
        ...
        self.search_cache.use_compression = search_config.get(
        ...
        environment = vector_config.get("environment") or os.getenv(
        ...
        if not api_key or not environment:
            ...
            raise VectorSearchError(
                "Pinecone API key and environment must be configured"
            )
    ...
    async def index_resource(
        ...
        with error_context("embedding_generation", resource_id=resource_id):
            ...
            if len(text_to_embed) > self.config.max_content_length:
                ...
                logger.warning(
                    f"Truncated content for resource {resource_id} to {self.config.max_content_length} characters"
                )
        ...
        with error_context("pinecone_upsert", resource_id=resource_id):
            # Upsert to Pinecone
            try:
                self.index.upsert(
                    vectors=[
    ...
    async def search_resources(
        ...
        with error_context("search_validation", query_length=len(query), top_k=top_k):
            # Validate inputs
        ...
        with error_context("query_embedding_generation", query_preview=query[:50]):
            try:
        ...
        with error_context(
            "pinecone_search", top_k=top_k, has_filters=bool(filter_metadata)
        ):
            try:
                # Search in Pinecone
                ...
                self.search_cache.set(
                    query, filter_metadata, top_k, formatted_results, user_id=user_id
                )
                ...
                logger.info(
                    f"Found {len(formatted_results)} results for query (search time: {search_time:.3f}s)"
                )
    ...
    def _update_avg_search_time(self):
        ...
        if self.metrics["search_requests"] > 0:
            self.metrics["avg_search_time"] = (
                self.metrics["total_search_time"] / self.metrics["search_requests"]
            )
    ...
    async def update_resource(
        ...
        await self.index_resource(resource_id, title, description, content, metadata)
    ...
    async def index_resource_chunks(
        ...
        try:
            ...
            if vectors_to_upsert:
                ...
                logger.info(
                    f"Successfully indexed {len(vectors_to_upsert)} chunks for resource {resource_id}"
                )
    ...
    async def search_with_reranking(
        ...
        initial_results = await self.search_resources(
            query, filter_metadata, min(rerank_top_k, 100)  # Cap at 100 for performance
        )
        ...
        try:
            ...
            for result in initial_results:
                ...
                title_boost = (
                    0.2 if any(term in title for term in query_lower.split()) else 0
                )
                desc_boost = (
                    0.1
                    if any(term in description for term in query_lower.split())
                    else 0
                )
    ...
    def _validate_content(self, content: str) -> None:
        ...
        if len(content) > self.config.max_content_length:
            raise MetadataValidationError(
                f"Content too long for embedding processing (max: {self.config.max_content_length})"
            )
    ...
    def _validate_query(self, query: str) -> None:
        ...
        if len(query) > self.config.max_query_length:
            raise SearchQueryError(
                f"Query too long (max: {self.config.max_query_length})"
            )
    ...
    def _validate_filter_metadata(self, filter_metadata: Dict) -> None:
        ...
        for key, value in filter_metadata.items():
            ...
            if isinstance(value, dict):
                raise MetadataValidationError(
                    "Invalid filter: nested objects not supported"
                )

            # Check for lists
            if isinstance(value, list):
                raise MetadataValidationError(
                    "Invalid filter: list values not supported"
                )

            # Check for null values
            if value is None:
                raise MetadataValidationError(
                    "Invalid filter: null values not supported"
                )
    ...
    def configure_cache(
        ...
        logger.info(
            f"Cache configured: search_enabled={enable_search_cache}, "
    ...
    async def warm_up_cache(
        self, common_queries: List[str], user_filters: Optional[List[Dict]] = None
    ):
        ...
        for query in common_queries:
            for filter_metadata in user_filters:
                try:
                    await self.search_resources(
                        query, filter_metadata=filter_metadata, top_k=5
                    )
                    total_warmed += 1
                    logger.debug(
                        f"Warmed up cache for query: {query[:50]}... with filter: {filter_metadata}"
                    )
                except Exception as e:
                    logger.warning(
                        f"Failed to warm up cache for query '{query[:50]}...' with filter {filter_metadata}: {e}"
                    )
    ...
    async def warm_up_embeddings_cache(self, common_texts: List[str]):
        ...
        for text in common_texts:
            try:
                ...
                logger.warning(
                    f"Failed to warm up embedding cache for text '{text[:50]}...': {e}"
                )
    ...
    def get_service_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            if (pinecone_healthy and cache_healthy)
            else "degraded",
        }
