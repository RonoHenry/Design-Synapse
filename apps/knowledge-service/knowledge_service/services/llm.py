...
from ..exceptions import LLMServiceError, EmbeddingGenerationError
...
class LLMService(ILLMService):
    ...
    def __init__(self, config = None):
        ...
        self.api_key = self.config.llm.get_provider_config("openai").get("api_key") or os.getenv("OPENAI_API_KEY")
        self.api_url = "https://api.openai.com/v1/chat/completions"
        self.model = self.config.llm.get_provider_config("openai").get("model", "gpt-3.5-turbo")
    ...
    @ErrorRecovery.with_retry(max_attempts=3, delay_seconds=1.0, exceptions=(httpx.TimeoutException, httpx.HTTPStatusError))
    async def generate_insights(
        ...
        with error_context(
            ...
            if len(text) < 50:
                logger.warning(f"Text content is very short ({len(text)} chars) for resource {resource.id}")
            
            # Check circuit breaker
            if not self.circuit_breaker.can_execute():
                raise LLMServiceError("LLM service temporarily unavailable (circuit breaker open)")
            ...
            if len(text) > max_content_length:
                logger.info(f"Truncated content from {len(text)} to {max_content_length} chars for resource {resource.id}")
            ...
            try:
                ...
                if response.status_code == 429:
                    ...
                    raise LLMServiceError(f"Rate limit exceeded. Retry after {retry_after} seconds")
                ...
                try:
                    ...
                    pass
                logger.error(f"OpenAI API error for resource {resource.id}: {e.response.status_code} - {error_detail}")
                raise LLMServiceError(f"LLM API error: {e.response.status_code}") from e
            except Exception as e:
                self.circuit_breaker.record_failure()
                logger.error(f"Unexpected error calling LLM API for resource {resource.id}: {e}")
                raise LLMServiceError(f"Failed to call LLM API: {str(e)}") from e
            
            # Parse and validate the response
            try:
                summary, key_takeaways, keywords = self._parse_insights_response(result, resource.id)
            except Exception as e:
                logger.error(f"Failed to parse LLM response for resource {resource.id}: {e}")
            ...
            try:
                ...
                logger.info(f"Successfully generated insights for resource {resource.id}")
    ...
    def _parse_insights_response(self, result: Dict, resource_id: int) -> Tuple[str, List[str], List[str]]:
        ...
        if len(sections) < 3:
            logger.warning(f"LLM response has fewer than 3 sections for resource {resource_id}, attempting to parse anyway")
            # Fallback parsing
    ...
    def _fallback_parse_insights(self, output: str, resource_id: int) -> Tuple[str, List[str], List[str]]:
        ...
        for line in lines:
            if any(marker in line.lower() for marker in ["key", "important", "takeaway", "conclusion"]):
                clean_line = line.lstrip("•-*123456789. ").strip()
    ...
    @ErrorRecovery.with_retry(max_attempts=2, delay_seconds=1.0, exceptions=(httpx.TimeoutException, httpx.HTTPStatusError))
            ...
            try:
                ...
                response = await self._client.post(
                    ...
                    json={
                        "model": self.model,
                        "messages": [
                            {"role": "system", "content": "Extract the main topics discussed in the content. Return them as a numbered list with one topic per line."},
    ...
    def _fallback_extract_topics(self, content: str) -> List[str]:
        ...
        technical_terms = {
            "architecture", "engineering", "construction", "design", "building", 
            "structure", "material", "concrete", "steel", "foundation", "safety",
            "code", "regulation", "standard", "specification", "analysis", "load",
            "seismic", "thermal", "mechanical", "electrical", "plumbing", "hvac"
        }
        ...
        with error_context("classify_resource", content_length=len(content)):
            ...
            response = await self._client.post(
                ...
                json={
                    "model": self.model,
                    "messages": [
                        {"role": "system", "content": "Classify the content into relevant DAEC technical categories. Return a numbered list."},
    ...
    def _fallback_classify_resource(self, content: str) -> List[str]:
        ...
        categories = []
        category_keywords = {
            "Structural Engineering": ["structure", "beam", "column", "foundation", "load", "stress"],
            "Architecture": ["design", "building", "space", "layout", "aesthetic", "plan"],
            "Construction": ["construction", "building", "material", "concrete", "steel", "site"],
            "Safety": ["safety", "hazard", "risk", "protection", "emergency", "code"],
            "Mechanical": ["hvac", "mechanical", "ventilation", "heating", "cooling", "system"],
            "Electrical": ["electrical", "power", "lighting", "circuit", "wiring", "energy"],
            "Plumbing": ["plumbing", "water", "pipe", "drainage", "sewer", "fixture"],
            "Codes & Standards": ["code", "standard", "regulation", "compliance", "requirement", "specification"]
        }
    ...
    def _fallback_extract_keywords(self, content: str) -> List[str]:
        ...
        stop_words = {
            ...
            "than", "when", "where", "why", "how", "what", "who", "which", "while", "during"
        }
    ...
    def compare_similarity(self, text1: str, text2: str) -> float:
        ...
        with error_context("compare_similarity", text1_length=len(text1), text2_length=len(text2)):
            ...
            try:
                ...
                raise EmbeddingGenerationError(f"Failed to compute text similarity: {str(e)}") from e
    ...
    async def batch_summarize(self, texts: List[str]) -> List[str]:
        ...
        with error_context("batch_summarize", batch_size=len(texts)):
            ...
            if failed_indices:
                logger.warning(f"Failed to summarize {len(failed_indices)} out of {len(texts)} texts")
        ...
        with error_context("detect_language", content_length=len(content)):
            ...
            response = await self._client.post(
                ...
                json={
                    "model": self.model,
                    "messages": [
                        {"role": "system", "content": "Detect the language of the given text. Respond with just the language name in English."},
    ...
    def get_service_health(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "service_status": "healthy" if self.circuit_breaker.state == "CLOSED" else "degraded"
        }