...
from common.config import (BaseServiceConfig, DatabaseConfig, Environment,
                           LLMConfig, LLMProvider, VectorConfig,
                           VectorProvider)
    ...
    def validate_supported_types(cls, v: str) -> str:
        ...
        for file_type in types:
            if file_type not in valid_types:
                raise ValueError(
                    f"Unsupported file type: {file_type}. Valid types: {', '.join(valid_types)}"
                )
...
class KnowledgeServiceSettings:
    ...
    def __init__(self):
        ...
        self.base = BaseServiceConfig(
            service_name="knowledge-service", service_version="1.0.0", port=8002
        )
        ...
        self.enable_auto_tagging: bool = (
            os.getenv("ENABLE_AUTO_TAGGING", "true").lower() == "true"
        )
        self.enable_summary_generation: bool = (
            os.getenv("ENABLE_SUMMARY_GENERATION", "true").lower() == "true"
        )
        self.enable_key_takeaways: bool = (
            os.getenv("ENABLE_KEY_TAKEAWAYS", "true").lower() == "true"
        )

        # Processing queue settings
        self.max_concurrent_processing: int = int(
            os.getenv("MAX_CONCURRENT_PROCESSING", "3")
        )
        self.processing_timeout_minutes: int = int(
            os.getenv("PROCESSING_TIMEOUT_MINUTES", "30")
        )

        # Enhanced batch processing settings
        self.batch_job_cleanup_hours: int = int(
            os.getenv("BATCH_JOB_CLEANUP_HOURS", "24")
        )
        self.batch_queue_check_interval_seconds: int = int(
            os.getenv("BATCH_QUEUE_CHECK_INTERVAL_SECONDS", "5")
        )
        self.batch_progress_update_interval_seconds: int = int(
            os.getenv("BATCH_PROGRESS_UPDATE_INTERVAL_SECONDS", "2")
        )
        self.batch_max_retry_attempts: int = int(
            os.getenv("BATCH_MAX_RETRY_ATTEMPTS", "3")
        )
        self.batch_enable_persistence: bool = (
            os.getenv("BATCH_ENABLE_PERSISTENCE", "true").lower() == "true"
        )
        ...
        self.max_content_length: int = int(
            os.getenv("MAX_CONTENT_LENGTH", "1000000")
        )  # 1MB of text
    ...
    def _validate_knowledge_service_config(self) -> None:
        ...
        if (
            ...
            or self.enable_key_takeaways
            or self.enable_auto_tagging
        ):
            if not self.llm.primary_provider:
                raise ValueError(
                    "LLM provider is required for content analysis features"
                )
