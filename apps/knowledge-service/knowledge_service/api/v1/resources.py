...
from packages.common.errors.base import (ConflictError, NotFoundError,
                                         ValidationError)

from ...core.config import settings
from ...core.error_handling import error_context
from ...core.observability import PerformanceMonitor, log_business_event
from ...exceptions import FileValidationError, PDFProcessingError
from ...infrastructure.database import get_db
from ...models import Bookmark, Resource, Topic
...
async def create_topic(
    ...
    """Create a new topic."""
    with error_context("create_topic", user_id=current_user.user_id, topic_name=name):
        # Validate input
        ...
        log_business_event(
            "topic_created",
            {"topic_id": topic.id, "topic_name": name, "has_parent": bool(parent_id)},
...
async def list_topics(
    db: Session = Depends(get_db), current_user: UserContext = Depends(get_current_user)
):
    """List all topics."""
    with error_context("list_topics", user_id=current_user.user_id):
        topics = db.query(Topic).all()

        # Log business event for analytics
        log_business_event(
            "topics_listed", {"topic_count": len(topics)}, user_id=current_user.user_id
        )
...
async def get_topic(
    topic_id: int, db: Session = Depends(get_db), _: int = Depends(get_current_user)
):
...
async def create_resource(
    ...
    if not resource.get("content_type") or not resource["content_type"] in [
        "pdf",
        "text",
        "url",
        "image",
...
async def list_resources(
    ...
    return {"total": total, "page": page, "limit": limit, "items": resources}
...
async def upload_file(
    ...
    if topic_ids:
        try:
            topic_id_list = [
                int(tid.strip()) for tid in topic_ids.split(",") if tid.strip()
            ]
            if topic_id_list:
    ...
    try:
        ...
        if content_type == "pdf":
            ...
            content_service = get_content_extraction_service()
            storage_path, file_size = await content_service.process_file(
                file, new_resource, db
            )
            logger.info(
                f"Successfully processed {content_type} file for resource {new_resource.id}"
            )
...
async def validate_file(
    file: UploadFile = File(...), _: int = Depends(get_current_user)
):
    ...
    content_service = get_content_extraction_service()
    validation_result["is_supported"] = content_service.is_supported_file_type(
        file.filename
    )
    ...
    validation_result["is_valid"] = (
        validation_result["is_supported"]
        and validation_result["is_valid_size"]
...
async def get_upload_config(_: int = Depends(get_current_user)):
...
async def get_job_progress(job_id: str, _: int = Depends(get_current_user)):
...
async def cancel_batch_job(
    job_id: str, db: Session = Depends(get_db), _: int = Depends(get_current_user)
):
...
async def get_batch_job_history(
    ...
    return {"total": len(history), "limit": limit, "jobs": history}
...
async def get_batch_processing_stats(_: int = Depends(get_current_user)):
