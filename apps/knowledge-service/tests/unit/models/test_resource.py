...
def test_create_topic_with_parent(db_session):
    ...
    parent = Topic(name="Architecture", description="General architecture topics")
...
def test_create_duplicate_topic(db_session):
    ...
    topic1 = Topic(name="Construction", description="Construction methods")
    db_session.add(topic1)
    db_session.commit()

    topic2 = Topic(name="Construction", description="Different description")
...
def test_create_resource(db_session):
    ...
    topic = Topic(name="BIM", description="Building Information Modeling")
    db_session.add(topic)
    db_session.commit()
...
def test_create_bookmark(db_session):
    ...
    bookmark = Bookmark(user_id=1, resource_id=resource.id, notes="Important resource")
...
def test_cascade_delete_resource(db_session):
    ...
    citation = Citation(
        resource_id=resource.id, project_id=1, context="Test citation", created_by=1
    )
    db_session.add(bookmark)
    db_session.add(citation)
    db_session.commit()
