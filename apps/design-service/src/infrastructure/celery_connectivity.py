...
def validate_broker_url(broker_url: str) -> bool:
    ...
    try:
        ...
        logger.debug(
            f"Broker URL validation successful: {parsed.scheme}://{parsed.netloc}"
        )
        return True
...
def _configure_serialization(app: Celery) -> None:
    ...
    app.conf.result_serializer = os.getenv(
        "CELERY_RESULT_SERIALIZER", DEFAULT_SERIALIZER
    )
...
def _configure_worker_settings(app: Celery) -> None:
    ...
    # Worker concurrency
    worker_concurrency_str = os.getenv(
        "CELERY_WORKER_CONCURRENCY", str(DEFAULT_WORKER_CONCURRENCY)
    )
    try:
        app.conf.worker_concurrency = int(worker_concurrency_str)
    except ValueError as e:
        logger.warning(
            f"Invalid CELERY_WORKER_CONCURRENCY, using default {DEFAULT_WORKER_CONCURRENCY}: {e}"
        )
        app.conf.worker_concurrency = DEFAULT_WORKER_CONCURRENCY

    # Worker prefetch multiplier
    worker_prefetch_str = os.getenv(
        "CELERY_WORKER_PREFETCH_MULTIPLIER", str(DEFAULT_WORKER_PREFETCH)
    )
    try:
        app.conf.worker_prefetch_multiplier = int(worker_prefetch_str)
    except ValueError as e:
        logger.warning(
            f"Invalid CELERY_WORKER_PREFETCH_MULTIPLIER, using default {DEFAULT_WORKER_PREFETCH}: {e}"
        )
    ...
    try:
        ...
        return {
            ...
            "worker": {
                ...
                "prefetch_multiplier": os.getenv(
                    "CELERY_WORKER_PREFETCH_MULTIPLIER", str(DEFAULT_WORKER_PREFETCH)
                ),
            },
            "serialization": {
                "task_serializer": os.getenv(
                    "CELERY_TASK_SERIALIZER", DEFAULT_SERIALIZER
                ),
