...
from ..infrastructure.celery_connectivity import (
    check_celery_health, get_celery_configuration_summary, validate_broker_url)
    ...
    try:
        ...
        if not app.conf.task_default_queue:
            ...
            validation_result["errors"].append(
                "Missing task_default_queue configuration"
            )
...
def check_worker_health(
    app: Optional[Celery] = None, timeout: int = HEALTH_CHECK_TIMEOUT
) -> Dict:
    """
            ...
            return {
                ...
                "summary": {
                    ...
                    "average_load_factor": round(
                        total_active_tasks / max(total_pool_size, 1), 2
                    ),
                },
...
def monitor_worker_performance(
    app: Optional[Celery] = None, duration_minutes: int = 5
) -> Dict:
    """
    ...
    try:
        ...
        monitoring_data["summary"] = {
            "initial_worker_count": initial_health.get("summary", {}).get(
                "worker_count", 0
            ),
            "initial_load_factor": initial_health.get("summary", {}).get(
                "average_load_factor", 0
            ),
