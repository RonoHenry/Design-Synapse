...
def generate_visuals_task(
    ...
    try:
        ...
        logger.error(
            f"Visual generation task {task_id} failed for design {design_id}: {e}"
        )
        ...
        if self.request.retries < self.max_retries:
            logger.info(
                f"Retrying visual generation task {task_id} (attempt {self.request.retries + 1})"
            )
            raise self.retry(
        ...
        if self.request.retries < self.max_retries:
            logger.info(
                f"Retrying visual generation task {task_id} due to unexpected error (attempt {self.request.retries + 1})"
            )
            raise self.retry(
                countdown=self.default_retry_delay * (2**self.request.retries)
            )
...
async def _generate_visuals_with_progress(
    ...
    await visual_service._update_design_visual_field(
        design_id=design_id, field="visual_generation_status", value="processing"
    )
    ...
    for i, visual_type in enumerate(visual_types):
        try:
            ...
            logger.info(
                f"Generating {visual_type} for design {design_id} (step {current_step}/{total_steps})"
            )
    ...
    generated_types = [
        vt for vt in visual_types if vt in results and "error" not in results[vt]
    ]
    failed_types = [
    ...
    await visual_service._update_design_visual_field(
        design_id=design_id, field="visual_generation_status", value=status
    )
...
def generate_single_visual_task(
    ...
    try:
        logger.info(
            f"Starting single visual generation task {task_id} for design {design_id}, type: {visual_type}"
        )
...
async def _run_single_visual_generation(
    task, design_id: str, visual_type: str, size: str, quality: str
) -> Dict[str, Any]:
    """
