...
class RuleEngine:
    ...
    def __init__(
        ...
        if rules_path is None:
            ...
            rules_path = os.path.join(
                os.path.dirname(__file__), "..", "..", "config", "building_codes"
            )
                ...
                for category_id, category in categories.items():
                    ...
                    for field in required_category_fields:
                        if field not in category:
                            errors.append(
                                f"Category '{category_id}' missing required field: {field}"
                            )
                ...
                for idx, rule in enumerate(rules):
                    ...
                    for field in required_rule_fields:
                        if field not in rule:
                            errors.append(
                                f"{rule_prefix}: Missing required field '{field}'"
                            )

                    # Validate severity
                    if (
                        "severity" in rule
                        and rule["severity"] not in self.VALID_SEVERITIES
                    ):
                        errors.append(
                    ...
                    if "building_types" in rule:
                        if not isinstance(rule["building_types"], list):
                            errors.append(
                                f"{rule_prefix}: 'building_types' must be a list"
                            )
                        elif len(rule["building_types"]) == 0:
                            errors.append(
                                f"{rule_prefix}: 'building_types' cannot be empty"
                            )
                    ...
                    if "category" in rule and "rule_categories" in rule_set:
                        ...
                        if category_id not in rule_set["rule_categories"]:
                            errors.append(
                                f"{rule_prefix}: Invalid category '{category_id}'. Must be defined in rule_categories"
                            )
                    ...
                    if "condition" in rule:
                        ...
                        if not isinstance(condition, dict):
                            errors.append(
                                f"{rule_prefix}: 'condition' must be a dictionary"
                            )
                            continue

                        if "type" not in condition:
                            errors.append(
                                f"{rule_prefix}: 'condition' missing 'type' field"
                            )
                        else:
                        ...
                        if "field" not in condition:
                            errors.append(
                                f"{rule_prefix}: 'condition' missing 'field' field"
                            )
                        ...
                        if condition_type in [
                            ...
                            if "value" not in condition:
                                errors.append(
                                    f"{rule_prefix}: '{condition_type}' condition missing 'value' field"
                                )
                            if "operator" not in condition:
                                errors.append(
                                    f"{rule_prefix}: '{condition_type}' condition missing 'operator' field"
                                )
                        elif condition_type == "required_field":
                            # For required_field, value or allowed_values should be present
                            if (
                                "value" not in condition
                                and "allowed_values" not in condition
                            ):
                                errors.append(
                                    f"{rule_prefix}: 'required_field' condition missing 'value' or 'allowed_values' field"
                                )
            ...
            if not isinstance(config, dict):
                ...
                if "severity_levels" in config:
                    ...
                    if not isinstance(severity_levels, dict):
                        errors.append(
                            "'validation_config.severity_levels' must be a dictionary"
                        )
                    else:
                        for severity, level_config in severity_levels.items():
                            if severity not in self.VALID_SEVERITIES:
                                errors.append(
                                    f"Invalid severity level '{severity}' in validation_config"
                                )
                            if not isinstance(level_config, dict):
                                errors.append(
                                    f"Severity level config for '{severity}' must be a dictionary"
                                )
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        cache_size = sum(len(json.dumps(data)) for data, _, _ in self._cache.values())
    ...
    def extract_parameter(
        self, design_spec: Dict[str, Any], path: str
    ) -> Optional[Any]:
        """
    ...
    def validate(
        self, design_spec: Dict[str, Any], rule_set: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        for rule in rules:
            ...
            if condition_type in ["minimum_value", "minimum_distance", "minimum_area"]:
                if (
                    ...
                    and operator == ">="
                    and current_value < required_value
                ):
...
class ValidationService:
    ...
    def validate_design(
        self, design: Design, validation_type: str, rule_set: str, user_id: int
    ) -> DesignValidation:
        """
        ...
        self.design_repo.update_design(design_id=design.id, status=new_status)
