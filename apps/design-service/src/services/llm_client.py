...
class LLMClient:
    ...
    def __init__(self, config: LLMConfig):
        ...
        if (
            ...
            or LLMProvider.OPENAI in self.fallback_providers
        ):
        ...
        if (
            ...
            or LLMProvider.GROQ in self.fallback_providers
        ):
            if Groq is None:
                raise ImportError("Groq client not installed. Run: pip install groq")
            self._groq_client = Groq(
                api_key=config.groq_api_key, timeout=config.timeout
            )
    ...
    async def generate_design_specification(
        self, description: str, building_type: str, requirements: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        user_prompt = self._build_design_user_prompt(
            description, building_type, requirements
        )
    ...
    async def generate_optimizations(
        self, design_specification: Dict[str, Any], optimization_types: List[str]
    ) -> Dict[str, Any]:
        """
        ...
        user_prompt = self._build_optimization_user_prompt(
            design_specification, optimization_types
        )
        ...
        for provider in providers:
            try:
                logger.info(
                    f"Attempting optimization generation with provider: {provider}"
                )
...
[
  {{
    ...
    def _calculate_confidence_score(
        self, specification: Dict[str, Any], requirements: Dict[str, Any]
    ) -> float:
        """
    ...
    async def generate_image(
        ...
        for provider in providers:
            try:
                ...
                if provider == LLMProvider.OPENAI:
                    ...
                    # Other providers not implemented yet
                    logger.warning(
                        f"Provider {provider} not implemented for image generation, skipping"
                    )
                    continue
                ...
                logger.warning(f"Timeout with provider {provider}: {e}")
                last_error = LLMTimeoutError(
                    f"Image generation timed out after {self.timeout} seconds"
                )
                continue
            except (APIError, RateLimitError) as e:
                logger.warning(f"API error with provider {provider}: {e}")
        ...
        # All providers failed
        error_msg = (
            f"All LLM providers failed for image generation. Last error: {last_error}"
        )
        logger.error(error_msg)
    ...
    def _enhance_image_prompt(self, prompt: str, image_type: str) -> str:
        ...
        if image_type == "floor_plan":
            ...
            enhanced += (
                "with room labels, dimensions, and standard architectural symbols. "
            )
