...
from src.api.v1.schemas.requests import (CommentCreateRequest,
...
async def verify_design_access(
    design_id: int, user_id: int, db: Session, project_client: ProjectClient
) -> Design:
    """
    ...
    try:
        await project_client.verify_project_access(
            project_id=design.project_id, user_id=user_id
        )
