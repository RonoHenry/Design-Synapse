...
class TaskStatusResponse(BaseModel):
    ...
    state: str = Field(
        ...,
        description="Current task state (PENDING, PROGRESS, SUCCESS, FAILURE, RETRY, REVOKED)",
    )
    result: Optional[Dict[str, Any]] = Field(
        None, description="Task result data (available when completed)"
    )
    ...
    next_retry: Optional[str] = Field(
        None, description="Next retry timestamp (ISO format)"
    )
    created_at: str = Field(..., description="Task creation timestamp (ISO format)")
    completed_at: Optional[str] = Field(
        None, description="Task completion timestamp (ISO format)"
    )
    execution_time_seconds: Optional[float] = Field(
        None, description="Task execution time in seconds"
    )
    worker_info: Dict[str, Any] = Field(
        default_factory=dict, description="Worker information"
    )
    ...
    try:
        ...
        if task_result.state == "PENDING":
            ...
            if hasattr(task_result, "date_done") and task_result.date_done:
                ...
                date_done = (
                    str(task_result.date_done) if task_result.date_done else None
                )
            ...
            # Task failed
            task_info["error"] = (
                str(task_result.result) if task_result.result else "Task failed"
            )
            task_info["progress"] = 0
            ...
            if task_result.info:
                ...
                task_info["error"] = (
                    str(task_result.result) if task_result.result else "Task retry"
                )
        ...
        if task_result.state in ["SUCCESS", "FAILURE"] and task_info.get(
            "completed_at"
        ):
            try:
...
async def get_task_status(
    ...
    try:
        logger.info(
            f"Getting task status for task {validated_task_id} (user: {current_user_id})"
        )
