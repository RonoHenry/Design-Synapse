...
class DesignResponse(BaseModel):
    ...
    generation_prompt: Optional[str] = Field(
        None, description="AI generation prompt used"
    )
    confidence_score: Optional[float] = Field(
        None, description="AI confidence score (0-100)"
    )
    ...
    parent_design_id: Optional[int] = Field(
        None, description="Parent design ID for versioning"
    )
    ...
    visual_generation_status: str = Field(
        ...,
        description="Visual generation status (pending, processing, completed, failed)",
    )
    visual_generation_error: Optional[str] = Field(
        None, description="Error message from visual generation"
    )
    visual_generated_at: Optional[datetime] = Field(
        None, description="Timestamp when visuals were generated"
    )
...
class OptimizationResponse(BaseModel):
    ...
    estimated_cost_impact: Optional[float] = Field(
        None, description="Estimated cost impact as percentage (negative = reduction)"
    )
    ...
    applied_at: Optional[datetime] = Field(
        None, description="When optimization was applied"
    )
    applied_by: Optional[int] = Field(
        None, description="User ID who applied optimization"
    )
...
class DesignFileResponse(BaseModel):
    ...
    file_type: str = Field(..., description="File type (pdf, dwg, dxf, png, jpg, ifc)")
...
class DesignCommentResponse(BaseModel):
    ...
    position_x: Optional[float] = Field(
        None, description="X coordinate for spatial annotation"
    )
    position_y: Optional[float] = Field(
        None, description="Y coordinate for spatial annotation"
    )
    position_z: Optional[float] = Field(
        None, description="Z coordinate for spatial annotation"
    )
