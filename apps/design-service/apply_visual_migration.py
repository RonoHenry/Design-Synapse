...
def main():
    ...
    with engine.connect() as conn:
        try:
            ...
            existing_visual_fields = [
                field for field in visual_fields if field in columns
            ]
    ...
    with engine.connect() as conn:
        ...
        conn.execute(
            text("INSERT INTO alembic_version (version_num) VALUES ('b6576f89ece0')")
        )
        conn.commit()
    ...
    if result.returncode == 0:
        ...
        with engine.connect() as conn:
            ...
            result = conn.execute(
                text(
                    "SHOW INDEX FROM designs WHERE Key_name = 'ix_designs_visual_generation_status'"
                )
            )
