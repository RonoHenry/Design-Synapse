...
def main():
    ...
    with engine.connect() as conn:
        ...
        existing_visual_fields = [
            field for field in visual_fields if field in columns_before
        ]
    ...
    if result.returncode == 0:
        ...
        with engine.connect() as conn:
            ...
            remaining_visual_fields = [
                field for field in visual_fields if field in columns_after
            ]

            if remaining_visual_fields:
                print(
                    f"❌ ERROR: Visual fields still present: {remaining_visual_fields}"
                )
            else:
        ...
        if result.returncode == 0:
            ...
            with engine.connect() as conn:
                ...
                final_visual_fields = [
                    field for field in visual_fields if field in columns_final
                ]
                ...
                if len(final_visual_fields) == len(visual_fields):
                    ...
                    print(
                        f"❌ ERROR: Missing visual fields: {set(visual_fields) - set(final_visual_fields)}"
                    )
        else:
