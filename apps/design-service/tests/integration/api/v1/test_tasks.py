...
class TestGetTaskStatus:
    ...
    def test_get_task_status_success(self, client, auth_headers):
        ...
        with patch("src.api.v1.routes.tasks.AsyncResult") as mock_result:
            ...
            assert (
                data["result"]["floor_plan_url"]
                == "https://cdn.example.com/designs/123/floor_plan.jpg"
            )
            assert (
                data["result"]["rendering_url"]
    ...
    def test_get_task_status_unauthorized(self, client_no_auth):
    ...
    def test_get_task_status_invalid_task_id(self, client, auth_headers):
        ...
        # Act
        response = client.get(f"/api/v1/tasks/{invalid_task_id}", headers=auth_headers)
    ...
    def test_get_task_status_with_metadata(self, client, auth_headers):
        ...
        with patch("src.api.v1.routes.tasks.AsyncResult") as mock_result:
            ...
            assert (
                data["metadata"]["task_name"]
                == "src.tasks.visual_generation.generate_visuals_task"
            )
...
class TestTaskStatusErrorHandling:
    ...
    def test_get_task_status_internal_server_error(self, client, auth_headers):
        ...
        with patch(
            "src.api.v1.routes.tasks.get_task_status_from_celery"
        ) as mock_get_status:
