...
class TestOptimizationRepository:
    ...
    def test_create_optimization(
        ...
        db_optimization = (
            db_session.query(DesignOptimization).filter_by(id=optimization.id).first()
        )
    ...
    def test_get_optimizations_by_design_id(
        ...
        DesignOptimizationFactory.create(
            db_session=db_session, design_id=design.id, optimization_type="cost"
        )
        DesignOptimizationFactory.create(
            db_session=db_session, design_id=design.id, optimization_type="structural"
        )
        DesignOptimizationFactory.create(
    ...
    def test_get_optimizations_by_design_id_filters_other_designs(
        ...
        DesignOptimizationFactory.create_batch(
    ...
    def test_update_optimization_status_apply(
        ...
        optimization = DesignOptimizationFactory.create(
            db_session=db_session, design_id=design.id, status="suggested"
        )
        db_session.commit()

        user_id = 42
        updated_optimization = repository.update_optimization_status(
            optimization.id, status="applied", user_id=user_id
        )
    ...
    def test_update_optimization_status_reject(
        ...
        updated_optimization = repository.update_optimization_status(
            optimization.id, status="rejected"
        )
    ...
    def test_update_optimization_status_not_found(
        ...
        updated_optimization = repository.update_optimization_status(
            99999, status="applied", user_id=1
        )
    ...
    def test_update_optimization_status_preserves_other_fields(
        ...
        # Update status
        updated_optimization = repository.update_optimization_status(
            optimization.id, status="applied", user_id=1
        )
    ...
    def test_get_optimizations_by_design_id_includes_all_statuses(
        ...
        DesignOptimizationFactory.create(
            db_session=db_session, design_id=design.id, status="applied"
        )
        DesignOptimizationFactory.create(
            db_session=db_session, design_id=design.id, status="rejected"
        )
        db_session.commit()
