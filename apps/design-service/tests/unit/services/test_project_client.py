...
import httpx
import pytest
from src.services.project_client import ProjectAccessDeniedError, ProjectClient
...
class TestVerifyProjectAccess:
    ...
    @pytest.mark.asyncio
    async def test_verify_project_access_user_is_member(
        self, project_client, mock_httpx_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_verify_project_access_user_not_member_404(
        self, project_client, mock_httpx_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_verify_project_access_user_not_member_403(
        self, project_client, mock_httpx_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_verify_project_access_project_not_found(
        self, project_client, mock_httpx_client
    ):
        ...
        mock_response.status_code = 404
        mock_response.raise_for_status.side_effect = httpx.HTTPStatusError(
            "Project not found", request=MagicMock(), response=mock_response
        )
        mock_httpx_client.request.return_value = mock_response
...
class TestGetProjectDetails:
    ...
    async def test_get_project_details_success(self, project_client, mock_httpx_client):
        ...
        mock_httpx_client.request.assert_called_once_with(
            "GET", f"http://localhost:8003/api/v1/projects/{project_id}", timeout=30.0
        )
