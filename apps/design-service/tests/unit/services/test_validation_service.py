...
from src.services.validation_service import RuleEngine, ValidationService
...
class TestValidationService:
    ...
    def test_validate_design_compliant(
        self, validation_service, compliant_design, rule_engine, db_session
    ):
    ...
    def test_validate_design_with_violations(
        self, validation_service, non_compliant_design, rule_engine, db_session
    ):
    ...
    def test_validation_completion_time(
        self, validation_service, compliant_design, rule_engine
    ):
        ...
        assert (
            elapsed_time < 10.0
        ), f"Validation took {elapsed_time:.2f} seconds, should be under 10 seconds"
        assert result is not None
...
class TestRuleEngine:
        ...
        return {
            ...
            "validation_config": {
                ...
                "severity_levels": {
                    ...
                    "info": {"blocks_approval": False, "color": "blue", "priority": 3},
                },
            },
        }
    ...
    def test_validate_compliant_design(self, rule_engine, sample_rule_set):
        ...
        design_spec = {
            ...
            "structure": {"wall_material": "concrete_block"},
            "compliance": {"setbacks": {"front": 5.0, "rear": 3.0, "side": 2.0}},
        }
    ...
    def test_validate_design_with_setback_violation(self, rule_engine, sample_rule_set):
        ...
        setback_violation = next(
            (v for v in result["violations"] if v["code"] == "SETBACK_FRONT"), None
        )
    ...
    def test_validate_design_with_multiple_violations(
        self, rule_engine, sample_rule_set
    ):
        ...
        design_spec = {
            ...
            "compliance": {
                "setbacks": {
                    ...
                    "rear": 2.5,
                    "side": 2.0,
                }  # Both front and rear violations
            },
        }
    ...
    def test_extract_nested_parameter(self, rule_engine):
        ...
        # Arrange
        design_spec = {"level1": {"level2": {"level3": {"value": 42}}}}

        # Act
        result = rule_engine.extract_parameter(
            design_spec, "level1.level2.level3.value"
        )
...
class TestRuleEngineCaching:
    ...
    def test_cache_invalidation_on_file_modification(
        self, rule_engine_with_cache, sample_rule_set
    ):
    ...
    def test_clear_cache_specific_rule_set(
        self, rule_engine_with_cache, sample_rule_set
    ):
...
class TestRuleConfigValidation:
    ...
    def test_load_valid_standard_building_code(self, rule_engine):
        ...
        try:
            ...
            pytest.skip(
                "Standard_Building_Code_2020.json not found in config directory"
            )
...
class TestRuleConfigValidation:
    ...
    def test_validate_invalid_severity(self, rule_engine):
        ...
        invalid_config = {
            ...
            "rule_categories": {
                "test": {"name": "Test Category", "description": "Test category"}
            },
            "rules": [
                {
                    "id": "TEST-001",
    ...
    def test_validate_required_validation_missing_value(self, rule_engine):
        ...
        assert len(errors) > 0
        assert any(
            "Missing required top-level field: validation_config" in err
            for err in errors
        )
...
class TestRuleConfigValidationNew:
    ...
    def test_validate_new_format_missing_metadata(self, rule_engine):
        ...
        # Arrange
        invalid_config = {"rule_categories": {}, "rules": [], "validation_config": {}}
        ...
        assert len(errors) > 0
        assert any(
            "Missing required top-level field: metadata" in err for err in errors
        )
    ...
    def test_validate_new_format_missing_metadata_fields(self, rule_engine):
        ...
        assert any(
            "Missing required metadata field: jurisdiction" in err for err in errors
        )
        assert any(
            "Missing required metadata field: effective_date" in err for err in errors
        )
        assert any(
            "Missing required metadata field: description" in err for err in errors
        )
    ...
    def test_validate_new_format_invalid_rule_category_reference(self, rule_engine):
        ...
        invalid_config = {
            ...
            "rule_categories": {
                "zoning": {"name": "Zoning", "description": "Zoning rules"}
            },
            "rules": [
                {
                    "id": "TEST_RULE",
            ...
            "validation_config": {
                "severity_levels": {"critical": {"blocks_approval": True}}
            },
        }
    ...
    def test_validate_new_format_invalid_condition_type(self, rule_engine):
        ...
        assert len(errors) > 0
        assert any(
            "Invalid condition type 'invalid_condition_type'" in err for err in errors
        )
    ...
    def test_validate_new_format_missing_condition_fields(self, rule_engine):
        ...
        assert any(
            "'minimum_value' condition missing 'value' field" in err for err in errors
        )
        assert any(
            "'minimum_value' condition missing 'operator' field" in err
            for err in errors
        )
...
class TestRuleEngineNewFormat:
    ...
    def test_validate_with_new_format_compliant_design(self, rule_engine):
        ...
        design_spec = {
            "building_info": {"type": "residential", "height": 7.0, "num_floors": 2},
            "setbacks": {"front": 6.0, "rear": 4.0, "side": 3.0},  # Compliant (>= 5.0)
        }
    ...
    def test_validate_with_new_format_violation(self, rule_engine):
        ...
        design_spec = {
            "building_info": {"type": "residential", "height": 7.0, "num_floors": 2},
            "setbacks": {"front": 4.0, "rear": 4.0, "side": 3.0},  # Violation (< 5.0)
        }
    ...
    def test_validate_building_type_filtering(self, rule_engine):
        ...
        design_spec = {
            ...
            "setbacks": {"front": 4.0},  # Would be violation for residential
        }
...
class TestValidationServiceWithNewFormat:
    ...
    @pytest.fixture
    def validation_service_with_new_format(
        self, validation_repo, design_repo, temp_rules_dir
    ):
    ...
    def test_validate_design_with_new_format(
        self, validation_service_with_new_format, compliant_design, db_session
    ):
        ...
        compliant_design.specification = {
            "building_info": {"type": "residential", "height": 7.0, "num_floors": 2},
            "setbacks": {"front": 6.0, "rear": 4.0, "side": 3.0},  # Compliant
        }
        db_session.commit()
    ...
    def test_validate_design_with_new_format_violations(
        self, validation_service_with_new_format, non_compliant_design, db_session
    ):
        ...
        non_compliant_design.specification = {
            "building_info": {"type": "residential", "height": 7.0, "num_floors": 2},
            "setbacks": {"front": 3.0, "rear": 4.0, "side": 3.0},  # Violation (< 5.0)
        }
        db_session.commit()
