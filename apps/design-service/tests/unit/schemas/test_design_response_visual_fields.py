...
class TestDesignResponseVisualFields:
    ...
    def test_design_response_with_visual_fields(self, db_session):
        ...
        assert response.visual_generated_at.replace(
            tzinfo=None
        ) == visual_generated_at.replace(tzinfo=None)
    ...
    def test_design_response_visual_fields_types(self, db_session):
        ...
        # Assert types
        assert (
            isinstance(response.floor_plan_url, str) or response.floor_plan_url is None
        )
        assert isinstance(response.rendering_url, str) or response.rendering_url is None
        assert (
            isinstance(response.model_file_url, str) or response.model_file_url is None
        )
        assert isinstance(response.visual_generation_status, str)
        assert (
            isinstance(response.visual_generation_error, str)
            or response.visual_generation_error is None
        )
        assert (
            isinstance(response.visual_generated_at, datetime)
            or response.visual_generated_at is None
        )
