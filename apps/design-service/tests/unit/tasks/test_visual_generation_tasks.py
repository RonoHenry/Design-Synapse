...
from src.services.llm_client import (LLMClient, LLMImageGenerationError,
                                     LLMTimeoutError)
...
class TestVisualGenerationTaskLogic:
    ...
    def test_task_configuration_values(self):
        ...
        assert (
            expected_single_config["name"] == "visual_generation.generate_single_visual"
        )
        assert (
            expected_cleanup_config["name"]
    ...
    def test_task_retry_logic(self, mock_task_instance):
        ...
        # Simulate retry call
        retry_countdown = mock_task_instance.default_retry_delay * (
            2**mock_task_instance.request.retries
        )
...
class TestRunVisualGeneration:
    ...
    @pytest.mark.asyncio
    @patch("src.tasks.visual_generation.get_settings")
    async def test_run_visual_generation_service_initialization_failure(
        self, mock_get_settings, mock_task
    ):
    ...
    async def test_run_visual_generation_progress_updates(self, mock_task):
        ...
        # Arrange
        with patch(
            "src.tasks.visual_generation.get_settings"
        ) as mock_get_settings, patch("src.tasks.visual_generation.LLMClient"), patch(
            "src.tasks.visual_generation.StorageClient"
        ), patch(
            "src.tasks.visual_generation.DesignRepository"
        ), patch(
            "src.tasks.visual_generation.VisualGenerationService"
...
class TestGenerateVisualsWithProgress:
    ...
    @pytest.mark.asyncio
    async def test_generate_visuals_with_progress_all_success(
        self, mock_task, mock_visual_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_visuals_with_progress_partial_failure(
        self, mock_task, mock_visual_service
    ):
        ...
        mock_visual_service.generate_3d_rendering.side_effect = VisualGenerationError(
            "Rendering failed"
        )
        mock_visual_service.generate_3d_model.side_effect = VisualGenerationError(
            "Model failed"
        )
