...
def upgrade() -> None:
    ...
    op.add_column(
        "designs", sa.Column("floor_plan_url", sa.String(length=500), nullable=True)
    )
    op.add_column(
        "designs", sa.Column("rendering_url", sa.String(length=500), nullable=True)
    )
    op.add_column(
        "designs", sa.Column("model_file_url", sa.String(length=500), nullable=True)
    )
    ...
    op.add_column(
        "designs", sa.Column("visual_generation_error", sa.Text(), nullable=True)
    )
    op.add_column(
        "designs", sa.Column("visual_generated_at", sa.DateTime(), nullable=True)
    )

    # Add index on visual_generation_status for performance
    op.create_index(
        op.f("ix_designs_visual_generation_status"),
        "designs",
        ["visual_generation_status"],
        unique=False,
    )
