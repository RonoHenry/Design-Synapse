...
class StorageClient:
    ...
    def __init__(
        ...
        if not bucket_name or not bucket_name.strip():
            raise StorageConnectionError(
                "Invalid storage configuration: bucket_name cannot be empty"
            )
        ...
        if not access_key or not access_key.strip():
            raise StorageConnectionError(
                "Invalid storage configuration: access_key cannot be empty"
            )
        if not secret_key or not secret_key.strip():
            raise StorageConnectionError(
                "Invalid storage configuration: secret_key cannot be empty"
            )
    ...
    def upload_file(
        self, file_obj: io.IOBase, key: str, content_type: str, compress: bool = False
    ) -> Dict[str, Any]:
        """
    ...
    def generate_signed_url(self, key: str, expiry_hours: int = 24) -> str:
        """
